// Q1 function intersection  - was writting jScript indexOF on JQ ELEMENT!!!
// Q2  When defining event listeners, will using $(.class) impact ALL items with that class, or only the one(s) impacted by the event
var Mta = {
...
function displayDestination($lineX) {
  $lineX.next().empty();
  $lineX.next().append('<option>Please select</option>');
  var hereLine = $lineX.val();
  Mta[hereLine].forEach(function(MtaStop){
    $lineX.next().append('<option>'+ MtaStop + '</option>');
  })
}
function displayJourney() {
  init_line = $('#line_start').val();
  init_stop = $('#station_start').val(); 
  end_line = $('#line_end').val();
